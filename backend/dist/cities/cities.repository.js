...
let CitiesRepository = class CitiesRepository {
    cities = [
        { cityId: 'city-mumbai-1', name: 'Mumbai' },
        { cityId: 'city-delhi-1', name: 'Delhi' },
        { cityId: 'city-jaipur-1', name: 'Jaipur' },
        { cityId: 'city-goa-1', name: 'Goa' },
        { cityId: 'city-kerala-1', name: 'Kerala' },
    ];
    create(data) {
        const city = { cityId: (0, uuid_1.v4)(), ...data };
    ...
    update(cityId, data) {
        const idx = this.cities.findIndex((c) => c.cityId === cityId);
        if (idx === -1)
            return undefined;
