...
let CitiesService = class CitiesService {
    ...
    findOne(cityId) {
        const c = this.citiesRepository.findById(cityId);
        if (!c)
            throw new common_1.NotFoundException(`City ${cityId} not found`);
        return c;
    }
    update(cityId, dto) {
        const updated = this.citiesRepository.update(cityId, dto);
        if (!updated)
            throw new common_1.NotFoundException(`City ${cityId} not found`);
        return updated;
    }
    remove(cityId) {
        if (!this.citiesRepository.delete(cityId))
            throw new common_1.NotFoundException(`City ${cityId} not found`);
        return { message: `City ${cityId} deleted` };
    }
};
